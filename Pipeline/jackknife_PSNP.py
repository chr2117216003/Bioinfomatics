...
kf = KFold(n_splits=len(positive_seq),shuffle=False)  
    ...
    for train_index , test_index in kf.split(positive_seq):  
        ...
        for jack_index,jack_data in enumerate(y_final_seq_value1):
            ...
            if jack_index==0:
                ...
                X_train1=np.concatenate([X_train1,np.array(y_final_seq_value1[0]).reshape(1,length)],axis=0)
                ...
                X_train1=np.concatenate([X_train1,np.array(y_final_seq_value1[1]).reshape(1,length)],axis=0)
